...
public sealed class ReplayRecorder : IDisposable
{
    ...
    {
        // porting-note(api13): Enumerable.Shuffle is .NET 10 only; api13 targets net9.
        // Random.Shared.Shuffle(Span<T>) exists on net9 and gives the same permutation semantics.
        readonly byte[] table = BuildShuffledTable();

        private static byte[] BuildShuffledTable()
        {
            var t = new byte[256];
            for (var i = 0; i < 256; ++i) t[i] = (byte)i;
            Random.Shared.Shuffle(t.AsSpan());
            return t;
        }
