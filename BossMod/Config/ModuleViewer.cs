...
public sealed class ModuleViewer : IDisposable
{
    ...
    public ModuleViewer(PlanDatabase? planDB, WorldState ws)
    {
        ...
        foreach (var info in BossModuleRegistry.RegisteredModules.Values)
        {
            ...
            try
            {
                (groupInfo, moduleInfo) = Classify(info);
            }
            catch (Exception ex)
            {
                // TODO(api12): module references Lumina row not present on game 7.1 (Quest/CFC/Fate added in 7.5+) — skip from viewer.
                Service.Log($"[ModuleViewer] skipping {info.ModuleType.FullName}: {ex.Message}");
                continue;
            }
