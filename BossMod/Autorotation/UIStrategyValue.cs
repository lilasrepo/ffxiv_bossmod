...
public class IntRenderer : IStrategyRenderer
{
    ...
    public bool DrawValue(StrategyContext context, StrategyConfig config, ref StrategyValue value)
    {
        ...
        ImGui.SetNextItemWidth(200 * ImGuiHelpers.GlobalScale);
        // API12 ImGuiNET has no SliderLong/InputLong; bridge via int (UI-bounded values fit comfortably).
        var fInt = (int)Math.Clamp(f, int.MinValue, int.MaxValue);
        if (cfg.Drag)
        {
            var minInt = (int)Math.Clamp(cfg.MinValue, int.MinValue, int.MaxValue);
            var maxInt = (int)Math.Clamp(cfg.MaxValue, int.MinValue, int.MaxValue);
            if (ImGui.SliderInt("", ref fInt, minInt, maxInt))
            {
                value = new StrategyValueInt() { Value = fInt };
                return true;
            }
        }
        else
        {
            if (ImGui.InputInt("", ref fInt, (int)cfg.Speed))
            {
                value = new StrategyValueInt() { Value = fInt };
                return true;
            }
