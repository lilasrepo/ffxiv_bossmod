...
public sealed class RotationModuleManager : IDisposable
{
    ...
    public void Update(float estimatedAnimLockDelay, bool isMoving, bool dutyRecorder)
    {
        ...
        // porting-note(api12/walk-back): upstream HEAD auto-injects the "VBM Multibox" preset (AutoTarget+Multibox+NormalMovement)
        // here whenever AIConfig.Enabled — that preset IS upstream's replacement-AI mover. This fork uses the legacy
        // AIManager/AIBehaviour/AIController instead, so the injection must NOT run: NormalMovement would write
        // Hints.ForcedMovement first and the legacy AIController's `if (ForcedMovement == null)` guard would then discard
        // the real melee/positional navigation (jerky dodging, melee profile not applied, won't path to target). Matches TC_ok.
