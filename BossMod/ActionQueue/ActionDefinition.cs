...
public sealed class ActionDefinitions
{
    ...
    // porting-note: HEAD registers actions that may only exist in game 7.5 sheets; degrade to default on missing rows so we
    // can early-return from registration without throwing. Callers must check RowId != 0 before treating data as real.
    private Lumina.Excel.Sheets.Action ActionData(uint id) => _actionsSheet.GetRowOrDefault(id) ?? default;
    private Lumina.Excel.Sheets.Item ItemData(uint id) => _itemsSheet.GetRowOrDefault(id % 500000) ?? default;
    private Lumina.Excel.Sheets.Trait TraitData(uint id) => _traitSheet.GetRowOrDefault(id) ?? default;
    ...
    public void RegisterSpell(ActionID aid, bool isPhysRanged = false, float instantAnimLock = 0.6f, float castAnimLock = 0.1f)
    {
        ...
        if (data.RowId == 0)
            return; // action absent from this game version's Action sheet (game 7.5-only)
    ...
    private void RegisterItem(ActionID aid, float animLock = 0.6f)
    {
        ...
        if (item.RowId == 0)
            return; // item absent from this game version's Item sheet
        var itemAction = item.ItemAction.Value;
        var spellId = (uint)itemAction.Type;
        ...
        _definitions[aidNQ] = new(aidNQ)
        {
            ...
            Range = range,
            CastTime = castTime,
            RequiresLineOfSight = false,   // TODO(api13): Lumina's ItemAction has no Action RowRef here
    ...
    private void RegisterBozja(BozjaHolsterID id)
    {
        ...
        // Upstream renamed ForbidExecute -> AllowExecute (ActionQueue.cs:122 reads AllowExecute
        // and ForbidExecute no longer exists), so take upstream's API. TC's ContainsKey guard
        // stays: the Bozja normal-action row is not guaranteed to be present in the TC data set,
        // and the bare indexer throws KeyNotFoundException while definitions are being built.
        if (id == BozjaHolsterID.LostSeraphStrike && _definitions.ContainsKey(normalAction))
    ...
    public void RegisterChargeIncreaseTrait(ActionID aid, uint traitId)
    {
        ...
        if (trait.RowId == 0 || !_definitions.ContainsKey(aid))
            return; // trait or aid absent from this game version
