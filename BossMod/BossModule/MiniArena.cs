...
public sealed class MiniArena(BossModuleConfig config, WPos center, ArenaBounds bounds)
{
    ...
    {
        // API12 ImGuiNET has no ImGui.CalcTextSizeA; bracket with PushFont and use CalcTextSize.
        var iconStr = icon.ToIconString();
        Vector2 size;
        using (ImRaii.PushFont(Service.IconFont))
            size = ImGui.CalcTextSize(iconStr);
        ImGui.GetWindowDrawList().AddText(Service.IconFont, fontSize, center - size / 2, color, iconStr);
    }
