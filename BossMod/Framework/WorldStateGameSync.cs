...
sealed class WorldStateGameSync : IWorldStateGameSync
{
    ...
    // nullable: HEAD-only hooks; not present on game 7.1
    private Hook<ProcessMapEffectNDelegate>? _processMapEffect1Hook;
    private Hook<ProcessMapEffectNDelegate>? _processMapEffect2Hook;
    private Hook<ProcessMapEffectNDelegate>? _processMapEffect3Hook;
    ...
    private Hook<ProcessLegacyMapEffectDelegate>? _processLegacyMapEffectHook;
    ...
    // nullable: HEAD-only — game-7.5 sig
    private Hook<ProcessPacketFateTradeDelegate>? _processPacketFateTradeHook;
    ...
    // nullable: HEAD-only — game-7.5 sig
    private Hook<InventoryAckDelegate>? _inventoryAckHook;

    private unsafe delegate void ProcessPacketPlayActionTimelineSync(Network.ServerIPC.PlayActionTimelineSync* data);
    // nullable: HEAD-only — game-7.5 sig
    private Hook<ProcessPacketPlayActionTimelineSync>? _processPlayActionTimelineSyncHook;
    ...
    public unsafe WorldStateGameSync(WorldState ws, ActionManagerEx amex)
    {
        ...
        // walk-back sig (shorter — same function, dropped trailing 7.5-specific bytes)
        ...
        // ProcessMapEffect: same prologue sig as walk-back's ProcessEnvControl. Body of detour may diverge between 7.1 / 7.5 — TODO(api12) verify packet layout.
        ...
        // HEAD-only ScanAllText for 3 mapEffectN handlers (game 7.5 split into 3 separate functions). On game 7.1 these don't exist as separate symbols.
        // Wrap in try/catch so plugin still loads if sig misses.
        try
        {
            ...
            {
                Service.Log($"[WSG] TODO(api12): ProcessMapEffectN sig matched {mapEffectAddrs.Length} addresses (expected 3); hooks disabled. game-7.1 has these inlined / under different layout.");
            }
        }
        catch (Exception ex)
        {
            Service.Log($"[WSG] TODO(api12): ProcessMapEffectN sig scan failed: {ex.Message}; hooks disabled.");
        }
        ...
        // HEAD-only — ProcessPacketFateTrade is a 7.5 addition. Wrap in try/catch.
        try
        {
        ...
        {
            Service.Log("[WSG] TODO(api12): ProcessPacketFateTrade sig not found; hook disabled (game-7.5 only).");
        }
        ...
        // HEAD-only — ProcessLegacyMapEffect is a 7.5 addition for backward-compat with old map-effect packets.
        try
        {
        ...
        {
            Service.Log("[WSG] TODO(api12): ProcessLegacyMapEffect sig not found; hook disabled (game-7.5 only).");
        }

        // HEAD-only — InventoryAck packet hook added in 7.5.
        try
        {
        ...
        {
            Service.Log("[WSG] TODO(api12): InventoryAck sig not found; hook disabled (game-7.5 only).");
        }

        // HEAD-only — ProcessPacketPlayActionTimelineSync added in 7.5.
        try
        {
        ...
        {
            Service.Log("[WSG] TODO(api12): ProcessPacketPlayActionTimelineSync sig not found; hook disabled (game-7.5 only).");
        }
    ...
    private unsafe void UpdateClient()
    {
        ...
        // TODO(api13): ActionManager.BeastmasterPets (7.5 job) does not exist in CS 6966 (TC game 7.20); BeastmasterBeasts stays empty
