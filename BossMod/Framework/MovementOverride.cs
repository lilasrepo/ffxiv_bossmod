...
public sealed unsafe class MovementOverride : BossMod.Services.IMovementOverride
{
    ...
    {
        // porting-note(api12/game-7.1): upstream HEAD's "add spinning support" (commit ffe64ca7a) reads
        // MoveControllerSubMemberForMine.Spinning at struct offset 0x94 (game-7.5-derived, UNVERIFIED on TC game 7.1 =
        // category-C struct drift) and gates the _forcedControlState reset on bAdditiveUnk. Kept reverted to TC_ok's proven
        // struct-free form (unconditional reset, void* self, no Spinning block). NOTE: the AutoDuty won't-approach + jitter
        // regression was NOT caused by this detour — root cause was ClientStructsEx.BaseMoveSpeed reading 0x7118 instead of
        // 0x7108 (MoveSpeed -> 0 -> ThetaStar gMultiplier 1/0 = +Inf; fixed + runtime-verified 2026-06-14). This revert is
        // kept only to avoid an unverified 7.5 offset for the niche Alzadaal's Legacy auto-spin feature (dropped as a B1;
        // restore the HEAD Spinning block + runtime-verify 0x94 on 7.1 if that one boss ever matters).
