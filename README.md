# Boss Mod（繁中移植版 · TC12） / Traditional-Chinese Port

> 別再倒在王的機制上。<br>
> Don't fail at raid mechanics anymore.

**繁體中文**：這是 **[Boss Mod](https://github.com/awgil/ffxiv_bossmod)** 的繁體中文客戶端移植版，對應 **FFXIV 7.1 / yanmucorp Dalamud API12（.NET 9）**。本專案僅做相容性移植，**非官方、非原作維護**；所有原始功能與設計著作權歸原作者 **veyn**。

**English**: A Traditional-Chinese-client port of **[Boss Mod](https://github.com/awgil/ffxiv_bossmod)** targeting **FFXIV 7.1 / yanmucorp Dalamud API12 (.NET 9)**. Compatibility port only — **unofficial and not maintained by the original author**. All original work © **veyn**.
...
3. 貼上下列網址並按 **+** 儲存：
   ...
   ```
4. 輸入 `/xlplugins`，搜尋 **Boss Mod (TC12)** → 安裝 → 啟用。
...
3. Add this URL and save with **+**:
   ...
   https://raw.githubusercontent.com/lilasrepo/DalamudPlugins/main/pluginmaster.json
   ```
4. Type `/xlplugins`, search **Boss Mod (TC12)** → Install → Enable.
